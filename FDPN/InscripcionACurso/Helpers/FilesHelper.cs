...
namespace FDPN.Helpers
{
    public class FilesHelper
    {
        ...
        private void UploadWholeFile(HttpContextBase requestContext, List<ViewDataUploadFilesResult> statuses)
        {
            ...
            List<string> nombrefotos = HttpContext.Current.Session["Fotos"] as List<string> ?? new List<string>();
            ...
            for (int i = 0; i < request.Files.Count; i++)
            {
                ...
                nombrearchivo = nombrearchivo.Replace(" ", "");
                nombrearchivo = nombrearchivo.Replace("-", "");
                string[] imageArray = nombrearchivo.Split('.');
                nombrearchivo = imageArray[0] + segundos.ToString() + "." + imageArray[1];
                nombrefotos.Add(nombrearchivo);
                HttpContext.Current.Session["Fotos"] = nombrefotos;
                var fullPath = Path.Combine(pathOnServer, nombrearchivo);
                ...
                if (imageArray.Length != 0)
                {
                    ...
                    else
                    {
                        ...
                        String fileThumb = Path.GetFileNameWithoutExtension(nombrearchivo) + "80x80.jpg";
