...
$(document).on("click", ".BtnInscribir", function (e) {
    ...
    if (disciplinaid === "") {
        showNotification("No ha seleccionado la disciplina", 4);
    }
    else {
        $("#" + trid).addClass('Inscribiendo'); //Volver la fila verde 
        $.ajax({
            url: '../inscripcion/_EventosDeLaDisciplina',
            type: 'GET',
...
$('.GrabarInscripciones').on('click', function (e) {
    ...
    if (disciplinaid === 4) {
        direccion = '../inscripcion/GrabarLasInscripcionesParaSincro';
    }
    ...
    $.ajax({
        ...
        type: 'POST',
        //dataType: 'JsonSendAfiliar',
        data: { deportistaid: deportistaid, Ids: Ids, marcas: marcas, disciplinaid: disciplinaid },
        async: false,
        success: function (result) {
